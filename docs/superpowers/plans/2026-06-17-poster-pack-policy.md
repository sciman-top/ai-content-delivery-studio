...
- [ ] Add built-in poster industry, renderer, and review-rubric pack IDs.
- [ ] Wire the poster workflow pack to those policy packs.
- [ ] Extend built-in catalog tests to assert the poster policy links.
...
- [ ] Update any pack-count or contract assertions affected by the poster scenario policy packs.
- [ ] Update backlog wording so the poster scenario slice is no longer outstanding once it lands.
