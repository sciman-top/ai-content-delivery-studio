...
- [ ] Add built-in article industry, renderer, and review-rubric pack IDs.
- [ ] Wire the article workflow pack to those policy packs.
- [ ] Extend built-in catalog tests to assert the article policy links.
...
- [ ] Update any pack-count or contract assertions affected by the article scenario policy packs.
- [ ] Update backlog wording so the article scenario slice is no longer outstanding once it lands.
