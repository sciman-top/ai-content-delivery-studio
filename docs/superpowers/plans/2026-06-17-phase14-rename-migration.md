...
**Historical note:** This plan documents the rename from the old `ImageSeriesStudio.*` paths to the current `ContentDeliveryStudio.*` paths; old paths below are historical only.
...
- [x] Define the slice as a mechanical Phase 14 rename closeout, not a feature slice.
- [x] Record the compatibility rule for legacy local app-data roots and historical old-name text.
- [x] Keep the note explicit about what old-name search hits remain acceptable.
...
- [x] Rename directories and project files in one mechanical pass.
- [x] Update solution project entries and project-reference paths to the new filenames.
- [x] Keep the repository buildable from the renamed tree only.
...
- [x] Replace active namespaces, type names, XAML class names, launch profile names, and telemetry identifiers with `ContentDeliveryStudio`.
- [x] Update test assertions, temp-folder names, repository-root detection, and shimmed build-lock paths to the new tree.
- [x] Preserve only the bounded legacy alias needed for local app-data-root fallback.
...
- [x] Add the legacy-root fallback in local app-data resolution.
- [x] Update current repo truth surfaces to describe the renamed internal solution honestly.
- [x] Mark the completed Phase 14 task items and remove stale rename backlog wording from current task/roadmap readouts.
