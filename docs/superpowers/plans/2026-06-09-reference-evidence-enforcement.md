...
- [x] Record the enforced change areas, acceptable evidence files, and local reference shelf priorities.
- [x] Keep the policy narrow: OpenAI provider, host/observability, persistence/schema, tooling/operator.
...
- [x] Detect changed files from the current worktree when no explicit path list is provided.
- [x] Map touched files to enforced areas.
- [x] Fail when an enforced area is touched without any acceptable evidence-file update.
- [x] Print actionable guidance listing touched files, acceptable evidence files, and recommended local reference directories.
...
- [x] Surface the policy document from top-level docs.
- [x] Add the verification script to the repo's normal local verification story.
- [x] Record the new enforcement slice in the task checklist.
...
- [x] Run the gate against the current docs-only change set and confirm it passes.
- [x] Run `git diff --check` on the modified docs and script.
- [x] Summarize remaining risks, especially that this is still a local gate and not yet CI-enforced.
