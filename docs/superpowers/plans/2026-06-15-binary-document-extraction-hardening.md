...
- [ ] **Step 1: Review the support matrix and reference basis**
...
- [ ] **Step 2: Keep task and roadmap wording aligned**
...
- [ ] **Step 3: Verify documentation-only consistency**
...
**Files:**
- Create: `src/ImageSeriesStudio.Infrastructure/Sources/LocalBinaryDocumentExtractionProvider.cs`
- Modify: `src/ImageSeriesStudio.Infrastructure/Sources/FakeDocumentExtractionProvider.cs` only if shared helper extraction is warranted
- Modify: `src/ImageSeriesStudio.Infrastructure/OpenAI/` no changes expected
- Modify: provider registration file only if the extraction provider is registered through DI today

- [ ] **Step 1: Add a failing extraction test for PDF**

Test file:

- `tests/ImageSeriesStudio.Tests/DocumentExtractionProviderTests.cs`
...
```powershell
dotnet test tests/ImageSeriesStudio.Tests/ImageSeriesStudio.Tests.csproj --filter "DocumentExtractionProviderTests"
```

Expected: failure because the new local provider does not exist yet.

- [ ] **Step 3: Add a failing extraction test for DOCX**

Acceptance:

- a real local provider extracts deterministic plain text with `paragraph` location hints from a text-bearing DOCX fixture

- [ ] **Step 4: Add the minimal provider implementation**

Implement:

- `src/ImageSeriesStudio.Infrastructure/Sources/LocalBinaryDocumentExtractionProvider.cs`
...
**Files:**
- Modify: `tests/ImageSeriesStudio.Tests/DocumentExtractionProviderTests.cs`
- Modify: `src/ImageSeriesStudio.Infrastructure/Sources/LocalBinaryDocumentExtractionProvider.cs`

- [ ] **Step 1: Add a failing bounded-scope failure test**
...
- [ ] **Step 2: Run the bounded failure test to verify it fails**
...
Expected: failure because the provider does not yet enforce the bounded-scope message.

- [ ] **Step 3: Add the minimal failure handling**

Requirements:

- no OCR fallback
- no fabricated extracted text
- explicit message that OCR/high-fidelity binary extraction is outside the current slice
...
- [ ] **Step 2: Run the stronger release-style gate**
...
- [ ] **Step 3: Update closeout wording in task and roadmap surfaces**
