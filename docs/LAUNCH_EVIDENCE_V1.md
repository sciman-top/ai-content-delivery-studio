...
| --- | --- | --- | --- |
| Fake-first primary workflow completes from planning through delivery. | Pass | `ProjectApplicationService_CompletesBriefBlueprintReviewAndDeliveryGoldenPathWithFakeProviders` and `tests/ContentDeliveryStudio.Tests/FakeWorkflowTests.cs` cover fake brief creation, blueprint promotion, prompt promotion, generation, structured review, human approval, delivery package export, and manifest output. | UI-level golden path still needs a launch rehearsal. |
| Article or plain-text supporting route creates evidence-backed illustration targets without paid providers by default. | Pass | `ProjectApplicationService_CompletesDocumentIllustrationSupportingRouteWithFakeProviders`, `tests/ContentDeliveryStudio.Tests/DocumentIllustrationWorkflowTests.cs`, and persistence tests cover fake-provider document planning, evidence anchors, approved target promotion, generation, structured review, human approval, delivery package export, manifest output, and stored plans. | Binary document extraction remains outside V1 launch scope. |
...
| OpenAI provider routing defaults are locked. | Pass | `OpenAiProviderContractTests`, `OpenAiSdkTextPlanningProvider` tests, and `V1LockedDefaultsTests` cover Responses for planning/review, Images for single-shot image generation, strict structured output schemas, `store: false`, and SDK-backed non-streaming text planning parity. ADR 0009 records the official .NET SDK adoption boundary. | Image generation and vision review SDK migration remain separate parity-tested tasks. |
...
- Official OpenAI .NET SDK evaluation is recorded in ADR 0009; non-streaming text planning now has an SDK-backed provider with parity tests, while image generation and vision review keep raw `HttpClient` until SDK migration preserves telemetry, routing, and artifact contracts.
