...
The image-generation path now establishes the first operator-controlled durable queue baseline: preparation and local queue mutations are separated from provider execution; queued and paused work survive reload; running work fails closed after interruption; retry creates a linked task instead of rewriting terminal history; and live-provider execution remains unauthorized by default.
...
- The gallery list enables WPF virtualization, recycling, deferred scrolling, keyboard/UIA selection, and asynchronous disk-cached thumbnails; a 1,000-row regression budget covers local service paths, while subjective scroll responsiveness and low-memory hardware remain manual/live verification.
