...
| The first real low-risk operator action runs end-to-end and writes audit output plus rollback or cleanup notes. | Historical snapshot evidence | The snapshot originally proved this through the then-present ToolAdapter/Operator test surface. That unused platform layer was later retired; this metric remains a fact about the locked snapshot and is not a claim that a current generic operator runtime exists. | A new operator capability requires a real user-visible consumer and fresh acceptance; do not restore the retired platform merely to preserve this historical metric. |
...
- Later repository gates do not refresh the live OpenAI sample or silently create a new V1 launch snapshot.
...
| Authorized operator trial | Accepted | Native WPF run `20260728-204423-261` was visibly inspected across all five workspaces and finalized under explicit user authority by reviewer `codex-agent-authorized-by-sciman`. The schema v2 record truthfully stores `operatorKind=authorized_agent`, `decisionAuthority=equivalent_operator_acceptance`, `liveAccepted=false`, reviewer/package/hash validation, and the committed summary is [20260728-scientific-figure-authorized-agent-acceptance.md](./change-evidence/20260728-scientific-figure-authorized-agent-acceptance.md). |
