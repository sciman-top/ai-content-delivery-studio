    ...
    private const string Schema = """
        {
          ...
          "$defs": {"claim":{"type":"object","additionalProperties":false,"properties":{"mergeKey":{"type":"string"},"category":{"type":"string","enum":["Definition","Mechanism","CausalRelation","ProcessStep","Comparison","QuantitativeResult","Constraint","Limitation","Uncertainty"]},"normalizedStatement":{"type":"string"},"sourceWording":{"type":"string"},"confidence":{"type":"number","minimum":0,"maximum":1},"status":{"type":"string","enum":["Draft","Accepted","Rejected"]},"sourceBlockId":{"type":"string"},"quotedText":{"type":"string"},"evidenceRole":{"type":"string","enum":["Support","Definition","Qualification","Contradiction"]},"evidenceValidationState":{"type":"string","enum":["Unvalidated","Validated","Rejected"]}},"required":["mergeKey","category","normalizedStatement","sourceWording","confidence","status","sourceBlockId","quotedText","evidenceRole","evidenceValidationState"]}}
        }
        """;
}
