...
- Treat six dimensions as jointly necessary, never as substitutes: **scientific correctness**, **physical meaning**, **immediate comprehensibility**, **concrete/structural realism**, **content-bearing richness**, and **counter-misreading protection**. A dense diagram with wrong physics, a beautiful photo with no causal relation, or a technically correct label card all fail.
- For apparatus, experiment, observation, and mechanism candidates, draw recognizable physical entities and their operational relations: for example an instrument body, terminals, controllable switch and hand; a transparent vessel, water, heater and thermometer; or a telescope tube, objective, eyepiece, distant object and observer eye. Their visible connections must carry the causal relationship. A list of named boxes is not a substitute for the apparatus.
- Make realism structural before making it photorealistic. A clean deterministic SVG can be realistic when geometry, material boundaries, supports, wires, rays, flow paths, and state changes are credible and traceable. Do not add texture, gloss, stock photos, or decorative complexity that obscures the physical mechanism.
- Require a concrete-object and causal-connection contract for every experimental/mechanism renderer. It must be machine-checkable where possible (semantic roles and named connections), then inspected in pixels. Counts of SVG primitives, labels, or cards never prove information richness by themselves.
...
- The source-evidence board is provenance only. It cannot supply the final figure's realism, scientific proof, causal participant, visual richness, or Gate 1 judgment. Inspect it separately from the explanatory candidates.
...
- In a hybrid figure, raster may depict texture, scene context, or non-authoritative material appearance only. Deterministic layers must carry every Chinese label, value, formula, unit, ray, arrow, wire, terminal, flow path, measurement direction, and other authoritative scientific relation. A raster layer is never accepted as the sole evidence of an exact physical connection.
